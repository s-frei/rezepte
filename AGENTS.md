...
- Everything in the repo is English (code, comments, commits, docs). UI copy lives only in `frontend/messages/*.json`, one catalog per language; `en.json` is the source and `de.json` is its equal, and both hold the same keys. → `docs/memory/content/conventions/svelte.mdx`
...
- UI uses only the design tokens from `frontend/src/app.css`; no raw colors, radii or fonts in components. → `docs/memory/content/architecture/design-system.mdx`
