#!/usr/bin/env bash
# Runs a command against `rezepte --demo` on :8070 with a throwaway data
# directory: builds nothing (the mise task depends on //service:build),
# starts the binary, waits for /healthz (seeding happens before it answers),
# runs the command, stops the binary and deletes the directory.
set -euo pipefail
...
# :8070 has to be free before we start. Our binary seeds for seconds before
...
if (exec 3<>/dev/tcp/127.0.0.1/8070) 2>/dev/null; then
	echo "with-demo: :8070 is already in use, stop that process first" >&2
	exit 1
fi
DATA_DIR="$(mktemp -d)"
...
env -u REZEPTE_ADMIN_USER -u REZEPTE_ADMIN_PASSWORD \
	REZEPTE_ADDR=:8070 REZEPTE_DATA_DIR="$DATA_DIR" REZEPTE_LOG_LEVEL=warn "$BIN" --demo &
PID=$!
...
for _ in $(seq 1 300); do
	if ! kill -0 "$PID" 2>/dev/null; then
		echo "with-demo: rezepte exited before answering on :8070" >&2
		exit 1
	fi
	curl -sf localhost:8070/healthz >/dev/null && break
	sleep 0.1
done
kill -0 "$PID" 2>/dev/null || { echo "with-demo: rezepte exited before answering on :8070" >&2; exit 1; }
curl -sf localhost:8070/healthz >/dev/null || { echo "with-demo: rezepte did not start on :8070" >&2; exit 1; }
"$@"
