...
<Tabs items={['service', 'frontend', 'docs']}>
  <Tab value="service">
    ...
    - `internal/demo` – seeds the sample recipes and placeholder images through those services for `--demo` (see [ADR 0013](/memory/decisions/0013-demo-mode-and-docs-pipeline))
  ...
  <Tab value="docs">
    ...
    Two independent Fumadocs apps, each with its own `package.json`:

    - `memory/` – this section, rendered by the app in `docs/` on port 4000
    - `user/` – a separate Fumadocs app on port 5000: `content/` (end-user pages; `api/reference.mdx` is generated, never hand-edited), `components/screenshot.tsx` (the `<Screenshot>` placeholder-fallback component), `screenshots/` (Playwright specs), `scripts/` (`with-demo.sh`, the OpenAPI fetch/generate and screenshot-audit scripts), `public/screenshots/` (committed PNGs)
    - `app/` – Fumadocs/Next.js glue per app, do not put content here
