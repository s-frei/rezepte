...
<Tabs items={['service', 'frontend', 'docs']}>
  ...
  <Tab value="frontend">
    ...
    - `messages/en.json`, `de.json` – UI copy, one catalogue per language
...
The SPA is Svelte 5 with runes and TypeScript throughout. Bits UI is used headless and styled by hand, so the visual direction stays a decision made in this repository rather than one inherited from a component library's own style system ([Design system](/architecture/design-system)). UI copy lives in Paraglide message files rather than in components, one catalogue per language (see [Languages](/features/languages)), which is what keeps the UI's two languages out of the code and the code itself English. [Svelte conventions](/conventions/svelte) owns the rules that follow from all three.
