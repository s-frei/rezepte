...
One process. `cmd/rezepte/main.go` loads config from `REZEPTE_*` variables, opens SQLite, runs migrations, ensures the instance owner, and starts `httpserver`. `--reset-superadmin-password` cuts that short: it sets the owner's password from `REZEPTE_ADMIN_PASSWORD`, drops every session of that account and returns without serving. The mux serves `/healthz`, the huma API under `/api/v1`, and the embedded SPA for everything else with an `index.html` fallback. Two prefixes never fall back: an unmatched route under `/api/` and anything under `/.well-known/` get a `404` problem body from `notFound` in `service/internal/httpserver/server.go` - the second because an MCP client that gets a `401` looks there for OAuth metadata, and the app shell with a `200` would read as a broken document rather than none.
...
<Tabs items={['service', 'frontend', 'docs']}>
  <Tab value="service">
    ...
    - `internal/mcpserver` – the MCP endpoint at `/mcp`, its tools calling the same services as the HTTP handlers (see [MCP server](/features/mcp))
