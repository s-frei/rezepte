...
- The connection DSN in `service/internal/db/db.go` sets the pragmas: `journal_mode(WAL)`, `busy_timeout(5000)`, `synchronous(NORMAL)`, `foreign_keys(ON)` and `_txlock=immediate`.
...
- `recipes.created_by` is `NOT NULL` without `ON DELETE`; deleting a user reassigns their recipes to the acting admin (see [Users and authentication](/features/users-and-auth)). Sessions cascade.
