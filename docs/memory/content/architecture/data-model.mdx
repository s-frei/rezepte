...
- `users.display_name` and `users.color` are both `NOT NULL`, and neither carries a `CHECK` while `role` and `locale` beside them do. `role` and `locale` are each a closed set the database itself has to hold: `role`'s values back `users_superadmin_idx` and the two triggers below, so a row outside the set would quietly make all three meaningless, and `locale`'s `CHECK (locale IN ('en', 'de'))` repeats the set `user.Locales` owns in Go - those two are the only places it exists (see [Languages](/features/languages)). Nothing in the schema reads `display_name` or `color`, and their rules live in Go instead - `normalizeDisplayName` and `ParseColor` in `service/internal/user/profile.go`, over the palette `user.Colors` owns. A `CHECK (color IN (…))` would be a second copy of that palette, and adding a ninth colour would then mean a migration as well as an edit. `NOT NULL` is the half the database does keep for both, and it is what lets every reader print a name without a fallback of its own: `user.Create` is the single writer of a user row and substitutes the username when no display name is given. Neither column is unique - `username` is the only thing that identifies an account (see [Users and authentication](/features/users-and-auth)).
