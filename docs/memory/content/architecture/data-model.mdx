...
- `ingredient_groups.position`, `ingredients.position` and `steps.position` are 0-based integers, rewritten sequentially in full on every recipe save (see [ADR 0009](/decisions/0009-recipes-as-documents-with-fts5)) - never edited in place, so there are no gaps or renumbering to reason about between saves.
...
- `recipes_fts` (contentless FTS5, `content=''`) is not shown above - it duplicates no data and has no foreign keys; it's a search index keyed by `recipes.rowid`, maintained by the recipe service rather than the schema. See [ADR 0009](/decisions/0009-recipes-as-documents-with-fts5).
- `recipes.created_by` is `NOT NULL` without `ON DELETE`; deleting a user reassigns their recipes to the acting admin (see [ADR 0011](/decisions/0011-admin-role-and-own-password-changes)). Sessions cascade.
