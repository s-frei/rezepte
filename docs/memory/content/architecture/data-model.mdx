...
erDiagram
  ...
  recipes ||--o{ steps : has
  steps ||--o{ step_references : has
  ingredients ||--o{ step_references : "named by"
  recipes ||--o{ recipe_tags : tagged
  tags ||--o{ recipe_tags : tags
  recipes ||--o{ images : has
  users ||--o{ favorites : stars
  recipes ||--o{ favorites : starred
  users {
    text id PK
    text username UK
    text display_name "shown wherever the UI names this person"
    text password_hash
    text role "superadmin, admin or user"
    text color "palette token"
    text locale "en or de, interface language"
    text created_at
    text updated_at
  }
  sessions {
  ...
  recipes {
    ...
    text updated_by FK
    text updated_at
    text edit_policy "nullable: open or locked"
  }
  instance_settings {
    integer id PK "always 1"
    boolean recipes_locked_by_default
  }
  ingredient_groups {
    text id PK
    text recipe_id FK
  ...
  step_references {
    text step_id PK "also FK to steps.id"
...
- `favorites` is keyed by `(user_id, recipe_id)` and both foreign keys cascade, so a star never outlives the user or the recipe it belongs to. The primary key already serves lookups by user; `favorites_recipe_idx` on `recipe_id` serves the reverse direction, which the per-page card lookup uses. See [Favorites](/features/favorites).
...
- `users.display_name`, `users.color` and `users.locale` are all `NOT NULL`, and none of them carries a `CHECK` while `role` beside them does. `role` is a closed set the database itself has to hold: its values back `users_superadmin_idx` and the two triggers below, so a row outside the set would quietly make all three meaningless. `locale` has no such reader, and its set lives in `service/internal/i18n/locales.json`; a `CHECK` would be a second copy, and adding a language would then take a migration (see [Languages](/features/languages)). Nothing in the schema reads `display_name`, `color` or `locale`, and their rules live in Go instead - `normalizeDisplayName`, `ParseColor` and `ParseLocale` in `service/internal/user/profile.go`, over the palette `user.Colors` owns. A `CHECK (color IN (…))` would be a second copy of that palette, and adding a ninth color would then mean a migration as well as an edit. `NOT NULL` is the half the database does keep for both, and it is what lets every reader print a name without a fallback of its own: `user.Create` is the single writer of a user row and substitutes the username when no display name is given. Neither column is unique - `username` is the only thing that identifies an account (see [Users and authentication](/features/users-and-auth)).
- `user.ErrNoSuperadmin` refuses to start against a non-empty `users` table without an owner, and it has to stay that strict: it is what turns a database without `users_superadmin_idx` and the two triggers into a refusal at startup rather than an instance running with no storage backstop, where the Go guards would be the only thing holding the invariant. Anything that relaxes it - promoting the first admin to owner on start, say - removes that net. A database whose `users` table lacks a column the current schema has fails on the query that finds the owner instead, loudly (`no such column`); the fix for both is to delete the file and let the migrations recreate it.
...
- `step_references` is keyed by `(step_id, word)`, following `recipe_tags`: a word carries at most one reference per step. Both foreign keys cascade, so deleting the step or the ingredient it names removes the reference with it - a dangling reference is a state the database cannot hold. See [Ingredient references](/features/ingredient-references) for the anchor it encodes and why it names an ingredient rather than a position.
...
- `recipes.edit_policy` is `NULL`, `'open'` or `'locked'` (a `CHECK` constraint); `NULL` means "follow the household setting" and is never written as a string. `instance_settings` holds that setting and exactly one row: `id INTEGER PRIMARY KEY CHECK (id = 1)`, inserted by the migration that creates the table, so reading it never has to handle "no row" and writing it is an `UPDATE`. It stands alone in the diagram because nothing references it. [Editing rights](/features/editing-rights) owns what the two columns decide.
...
Queries are hand-written SQL under `service/internal/db/queries/`, compiled by sqlc into typed Go under `service/internal/db/sqlc/`, and the generated code is committed. Writing the SQL by hand is what keeps FTS5 and the recipe aggregate's whole-document write expressible at all; generating the scanning is what makes the compiler check that a query's columns and the struct receiving them still agree after an edit. Committing the output means a checkout builds without sqlc installed, and a query changed without regenerating shows up as a diff the gate fails on rather than as a surprise at runtime. A handful of statements sqlc cannot express are hand-written in the same package against the same `DBTX`; [Recipes and search](/features/recipes) lists them and says why.

Within one statement, parameters are either all `sqlc.arg()` or all plain `?`. `sqlc.arg()` compiles to an explicitly numbered `?N`, SQLite numbers the plain placeholders after the highest explicit index, and the driver never binds them — a silent mismatch rather than an error at generation or at run time.
